...
async function addMember() {
  ...
  // Save to DB
  const res = await apiPost('save_member', newMember);
  if (res.error) {
...
async function delMember(id) {
  ...
  // Delete from DB
  const res = await apiDelete('delete_member', id);
  if (res.error) {
...
async function saveMemberEdit(id) {
  ...
  // Save to DB
  const res = await apiPost('save_member', m);
  if (res.error) {
  ...
  tb.innerHTML = members.map((m, i) => {
    ...
    return `
      <tr>
        ...
        <td style="white-space:nowrap;">
          <button class="btn btn-sm btn-outline" title="Edit"   onclick="startMemberEdit('${m.id}')">✏️</button>
          <button class="btn btn-sm btn-danger"  title="Delete" onclick="delMember('${m.id}')">🗑️</button>
        </td>
      </tr>`;
  }).join('');
